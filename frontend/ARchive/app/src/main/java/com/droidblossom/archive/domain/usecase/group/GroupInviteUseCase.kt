...
class GroupInviteUseCase @Inject constructor(
    private val repository: GroupRepository
) {
    suspend operator fun invoke(request : InviteGroupRequestDto) =
        flow {
            try {
                emit(repository.postGroupInviteRequest(request).onSuccess {
